...
const IntelligenceCenterView = () => {
  ...
  const [annealingStep, setAnnealingStep] = useState(0)
  ...
  useEffect(() => {
    ...
    const originalSkillsCount = selectedAttritionEmp.covariates_explain.find(c => c.factor.includes("Skill"))?.val ?? 5
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const future_baseline = 0.04 + 0.10 * peak_1yr + 0.06 * peak_3yr
      
      cumulativeHazard += future_baseline * nextHazardMultiplier
      const survivalProb = Math.exp(-cumulativeHazard)
      
      nextForecast.append ? null : nextForecast.push({
        month: m,
        survival_probability: Math.max(0.01, Math.min(1.0, survivalProb)),
        attrition_probability: Math.max(0.0, Math.min(0.99, 1.0 - survivalProb))
      })
    }
  ...
  useEffect(() => {
    if (selectedAttritionEmp) {
      ...
      const skillCov = selectedAttritionEmp.covariates_explain.find(c => c.factor.includes("Skill"))
  ...
  useEffect(() => {
    ...
    const tick = () => {
      setNodesState(prev => {
        ...
        const nodeMap = {}
        prev.forEach((n, i) => { nodeMap[n.id] = i })

        // Clone nodes to update physics positions
        const nextNodes = prev.map(n => ({ ...n, vx: n.vx * 0.85, vy: n.vy * 0.85 }))
  ...
  const handleNodeMouseDown = (nodeId, e) => {
    ...
    const updateCoords = (moveEvent) => {
      ...
      setNodesState(prev => prev.map(n => n.id === nodeId ? { ...n, x: Math.max(20, Math.min(480, x)), y: Math.max(20, Math.min(480, y)), vx: 0, vy: 0 } : n))
    }
  ...
  const fetchCareerEmployees = async () => {
    try {
      const API_BASE = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000'
      const token = localStorage.getItem('auth_token') || ''
  ...
  const loadCareerPath = async (empId) => {
    if (!empId) return
    try {
  ...
  const addSkillMatchReq = () => {
    ...
    setMatchSkillsInput([...matchSkillsInput, { name: newSkillName.trim(), level: Number(newSkillLevel) }])
    setNewSkillName('')
  }
  ...
  const triggerSkillMatch = async () => {
    ...
    try {
      ...
      const results = await apiCall('/skill-match', 'POST', { target_skills: matchSkillsInput })
  ...
  const triggerTeamOptimize = async () => {
    ...
    try {
      ...
      const mockNamesPool = [
        "Aurelia Vance", "Cyrus Sterling", "Silas Thorne", "Maeve Brooks", "Julian Cross", 
        "Dante Blackwell", "Eleni Moretti", "Leona Mercer", "Gideon Cole", "Fiona Hayes"
      ]
      ...
      const intervalId = setInterval(() => {
        if (currentSimIndex >= simulationSteps) {
          ...
          const stepData = history[currentSimIndex] || history[history.length - 1]
  ...
  const getHighlightPathNodes = () => {
    const activeMatch = matchResults.find(r => r.employee_id === activeMatchEmployeeId)
  ...
  return (
    <div className="min-h-full pb-10">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-white/5 pb-6">
        <div className="text-left">
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/5 px-3 py-1 text-[9px] font-black uppercase tracking-widest text-indigo-400 mb-2">
            <Cpu size={10} className="animate-spin-slow" /> Math-Engine & Optimization
          </div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 text-white">Intelligence Center</h1>
          <p className="text-slate-400 text-sm leading-relaxed max-w-3xl">
            Aurelius state-of-the-art decision workbench. Powered by graph theory, combinatorial solvers, survival models, and Markov transition matrices.
          </p>
        </div>
      </header>
      ...
      <div className="premium-card p-5 border border-indigo-500/20 bg-indigo-500/5 mb-6 relative overflow-hidden text-left">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-bold text-white mb-1 uppercase tracking-wider">Executive Decision Workbench (Math-Engine)</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5">
                <div className="font-extrabold text-[10px] text-indigo-400 uppercase tracking-wider mb-1">1. Semantic Skills Graph</div>
              ...
              <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5">
                <div className="font-extrabold text-[10px] text-indigo-400 uppercase tracking-wider mb-1">2. Optimal Team Assembly</div>
                <p className="text-[10px] text-slate-400 leading-normal">
              ...
              <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5">
                <div className="font-extrabold text-[10px] text-indigo-400 uppercase tracking-wider mb-1">3. Attrition Sandbox Simulator</div>
                <p className="text-[10px] text-slate-400 leading-normal">
      ...
      <div className="flex flex-wrap gap-2 border-b border-white/5 pb-4 mb-6">
        {[
          { id: 'skill-match', label: 'Semantic Skills Graph', icon: <Brain size={14} /> },
          { id: 'team-builder', label: 'Optimal Team Assembly', icon: <Zap size={14} /> },
          { id: 'attrition', label: 'Survival Attrition', icon: <TrendingUp size={14} /> },
          { id: 'ona', label: 'Network Analysis (ONA)', icon: <Users size={14} /> },
          { id: 'career-path', label: 'Markov Career Path', icon: <GitBranch size={14} /> }
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="relative">
        <AnimatePresence mode="wait">
          ...
          {activeSubTab === 'skill-match' && (
            <motion.div key="skill-match" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 text-left">
              {/* Left Settings */}
              <div className="space-y-6">
                <div className="premium-card p-5 border border-white/5 bg-slate-950/40 backdrop-blur-md">
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300 mb-4 border-b border-white/5 pb-2 flex items-center gap-2">
                    <Briefcase size={14} className="text-indigo-400" /> Define Target Requirements
                  </h3>
                  
                  {/* Skill Add Input */}
                  <div className="space-y-3 mb-6">
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1 block">Skill Node</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1 block">Min Proficiency</label>
                      <select
                        value={newSkillLevel}
                  ...
                  <div className="space-y-2 max-h-[180px] overflow-y-auto custom-scrollbar border-t border-white/5 pt-3">
                    {matchSkillsInput.map((skill, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-white/5 border border-white/5 rounded-xl px-3 py-2">
                        <div className="text-xs">
                          <span className="font-bold text-white">{skill.name}</span>
                          <span className="text-indigo-400 ml-2">Lvl {skill.level}</span>
                        </div>
                        <button onClick={() => removeSkillMatchReq(idx)} className="text-rose-400 hover:text-rose-300">
                          <Trash2 size={12} />
                        </button>
                      </div>
                    ))}
                    {matchSkillsInput.length === 0 && (
                      <div className="text-xs text-slate-500 text-center py-4">No skills requirements added yet.</div>
                    )}
                  </div>
                  ...
                  >
                    <Search size={14} /> {matchingLoading ? 'Graph Traversing...' : 'Solve Adjacencies'}
                  </button>
                </div>
              </div>
              ...
              <div className="premium-card p-6 border border-white/5 bg-slate-950/20">
                <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-6">
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">Semantic Matching Matrix & Path Analysis</h3>
                  <span className="text-[10px] text-slate-500">Shortest Path Dijkstra Weighting</span>
                </div>
                ...
                {matchResults.length > 0 ? (
                  <div className="grid grid-cols-1 xl:grid-cols-[250px_1fr] gap-6">
                    ...
                    <div className="space-y-2 border-r border-white/5 pr-4">
                      {matchResults.map((result) => (
                        ...
                        >
                          <div className="font-bold text-white text-xs">{result.full_name}</div>
                          <div className="text-[9px] text-slate-400 mt-1 uppercase tracking-wider">{result.role}</div>
                          <div className="flex items-center justify-between mt-3 border-t border-white/5 pt-2">
                            ...
                            <span className="text-xs font-black text-primary">{(result.match_details.overall_compatibility * 100).toFixed(0)}%</span>
                          </div>
                        </button>
                      ))}
                    </div>
                        ...
                        return (
                          <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6">
                            ...
                            <div className="space-y-4">
                              <div>
                                <div className="text-[9px] uppercase tracking-widest text-slate-500 font-bold mb-1">Target Match Breakdown for:</div>
                                <h4 className="text-lg font-extrabold text-white">{activeMatch.full_name}</h4>
                              </div>

                              <div className="space-y-3">
                                {activeMatch.match_details.detailed_matches.map((detail, idx) => (
                                  <div key={idx} className="rounded-xl border border-white/5 bg-slate-950 p-4">
                                    <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
                                      <div className="text-xs font-bold text-white">Target Skill: {detail.target_skill} (L{detail.target_level})</div>
                                      <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${
                                        detail.status === 'Perfect' ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' :
                                        detail.status === 'Highly Transferable' ? 'text-cyan-400 border-cyan-500/20 bg-cyan-500/5' :
                                        detail.status === 'Trainable Gap' ? 'text-amber-400 border-amber-500/20 bg-amber-500/5' : 'text-rose-400 border-rose-500/20 bg-rose-500/5'
                                      }`}>{detail.status}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center flex-wrap gap-2 text-xs">
                                      {detail.matched_by_skill ? (
                                          ...
                                          {detail.semantic_distance > 0 && (
                                            <>
                                              <div className="text-slate-500 flex flex-col items-center">
                                                <span className="text-[8px] text-indigo-400 font-mono">Weight: {detail.semantic_distance}</span>
                                        ...
                                        <span className="text-rose-400 font-mono text-[10px]">No transition path discovered. Distance: Infinite.</span>
                                      )}
                                    </div>
                                  </div>
                                ))}
                            ...
                            <div className="rounded-xl border border-white/5 bg-slate-950 p-4 flex flex-col justify-between">
                              <div>
                                <div className="text-[9px] uppercase tracking-widest text-slate-500 font-bold mb-2">Shortest Path Graph View</div>
                                <div className="text-[10px] text-slate-400 leading-relaxed mb-4">
                                  Dijkstra nodes in neon green are present in candidate profile. Lines light up in cyan showing path transitions.
                                </div>
                              </div>

                              <div className="relative h-72 border border-white/5 rounded-lg overflow-hidden bg-slate-950/80">
                                <svg className="absolute inset-0 h-full w-full pointer-events-none" viewBox="0 0 1000 550">
                                  {/* Links */}
                                  {SKILL_GRAPH_LINKS.map((link, idx) => {
                                    ...
                                    const isActivePath = highlightNodes.has(link.source) && highlightNodes.has(link.target)
                                    ...
                                    return (
                                      <g key={idx}>
                                        <line
                                          ...
                                          x2={tgt.x}
                                          y2={tgt.y}
                                          stroke={isActivePath ? "#2dd4bf" : "#ffffff"}
                                          strokeOpacity={isActivePath ? 0.9 : 0.05}
                                  ...
                                  {/* Nodes */}
                                  {Object.entries(SKILL_GRAPH_COORDS).map(([name, node]) => {
                                    const isHighlighted = highlightNodes.has(name)
                                    ...
                                    return (
                                      <g key={name}>
                                        ...
                                        <text
                                          ...
                                          fontSize="13"
                                          fontWeight={isHighlighted ? "black" : "normal"}
                                          textAnchor="middle"
                                        >
                                          {name}
                                        </text>
                  ...
                  <div className="text-slate-400 text-sm py-20 text-center">
                    Enter target requirements and click solve to run graph shortest-path matching.
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeSubTab === 'team-builder' && (
            <motion.div key="team-builder" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 text-left">
              {/* Left Config */}
              <div className="space-y-6">
                <div className="premium-card p-5 border border-white/5 bg-slate-950/40 backdrop-blur-md">
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300 mb-4 border-b border-white/5 pb-2">Combinatorial Constraints</h3>
                  
                  {/* Inputs */}
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1.5 block">Budget Cap (CFO Limit)</label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1.5 block">Max Team Size</label>
                      <input
                        type="number"
                        min="2"
                        max="6"
                    ...
                    {/* Skills Requirements */}
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-2 block">Skill Matrix Demands</label>
                      <div className="flex gap-2 mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const inputEl = document.getElementById('team-skill-name')
                            if (inputEl && inputEl.value.trim()) {
                              setTeamSkillsInput([...teamSkillsInput, { name: inputEl.value.trim(), level: 3 }])
                              inputEl.value = ''
                            }
                          }}
                      ...
                      <div className="space-y-1.5 max-h-[140px] overflow-y-auto custom-scrollbar">
                        {teamSkillsInput.map((skill, idx) => (
                          <div key={idx} className="flex items-center justify-between bg-white/5 rounded-lg px-2.5 py-1 text-xs">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                {[1,2,3,4,5].map(v => <option key={v} value={v}>L{v}</option>)}
                              </select>
                              <button onClick={() => setTeamSkillsInput(teamSkillsInput.filter((_, i) => i !== idx))} className="text-rose-400 text-xs">×</button>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  >
                    <Play size={14} /> {optimizingLoading ? 'Simulated Annealing Run...' : 'Find Mathematically Perfect Team'}
                  </button>
                </div>
              </div>
              ...
              <div className="premium-card p-6 border border-white/5 bg-slate-950/20 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-6">
                    <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">Optimization Assembly Results</h3>
                    <span className="text-[10px] text-slate-500">Metropolis Hastings Simulated Annealing</span>
                  </div>
                  ...
                  {annealingStatus === 'running' && (
                    <div className="py-12 flex flex-col items-center justify-center space-y-6">
                      <div className="text-center">
                        <div className="text-xs font-mono text-primary uppercase tracking-[0.2em] mb-2 animate-pulse">Running Simulated Annealing Model</div>
                        <div className="text-3xl font-black text-white font-mono">Temp: {annealingTemp.toFixed(2)}K</div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {shuffledTeamNames.map((name, i) => (
                          <div key={i} className="px-3 py-2 bg-slate-900 border border-white/10 text-slate-300 font-mono text-[10px] uppercase rounded-lg animate-pulse">
                            {name}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        <div className="rounded-xl border border-white/5 bg-slate-950 p-4 relative overflow-hidden">
                          ...
                          <div className="text-[9px] uppercase font-bold tracking-widest text-slate-500 mb-1">Total Team Cost</div>
                          <div className="text-2xl font-black text-white">${optimizedTeam.metrics.total_cost.toLocaleString()}</div>
                          <div className="flex items-center gap-2 mt-2">
                            <span className={`h-2 w-2 rounded-full ${optimizedTeam.metrics.is_under_budget ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]' : 'bg-rose-400'}`} />
                            <span className="text-[10px] text-slate-400">{optimizedTeam.metrics.is_under_budget ? 'Under CFO Budget Cap' : 'Exceeds budget cap'}</span>
                          </div>
                        </div>

                        <div className="space-y-2">
                          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1">Assembly Roster</div>
                          {optimizedTeam.optimized_team.map((emp) => (
                            <div key={emp.id} className="rounded-xl border border-white/5 bg-white/2 p-3 flex items-center justify-between hover:bg-white/[0.04] transition-all">
                              <div>
                                <div className="font-bold text-white text-xs">{emp.full_name}</div>
                                <div className="text-[9px] text-slate-400 uppercase tracking-wider mt-0.5">{emp.role}</div>
                              </div>
                              <div className="text-xs font-black text-slate-400">${emp.estimated_cost.toLocaleString()}/yr</div>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                          <div className="flex justify-between items-center mb-3">
                            <div className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Convergence Timeline</div>
                            <span className="text-[9px] font-mono text-cyan-300">Annealing Steps: {optimizedTeam.total_optimization_steps}</span>
                          </div>
                          ...
                          <div className="h-28 w-full border-b border-l border-white/5 relative bg-black/20 rounded">
                            <svg className="absolute inset-0 h-full w-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                              <polyline
                                ...
                                stroke="#2dd4bf"
                                strokeWidth="2.5"
                                points={annealingHistory.map((h, i, arr) => {
                                  const x = (i / (arr.length - 1)) * 100
                                  const energies = arr.map(item => item.energy)
                                  const minE = Math.min(...energies)
                                  const maxE = Math.max(...energies)
                                  const range = maxE - minE || 1
                                  const y = 90 - ((h.energy - minE) / range) * 80
                                  return `${x},${y}`
                                }).join(' ')}
                              />
                            </svg>
                            <div className="absolute right-2 bottom-1 text-[8px] text-slate-600 uppercase font-mono">Cooling Step</div>
                            <div className="absolute left-2 top-1 text-[8px] text-slate-600 uppercase font-mono">Energy Metric</div>
                          </div>
                        </div>

                        {/* Skill Coverage details */}
                        <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                          <div className="text-[9px] uppercase font-bold tracking-widest text-slate-500 mb-2">Total Skill coverage</div>
                          <div className="text-3xl font-extrabold text-indigo-400 mb-2">{optimizedTeam.metrics.coverage_percentage}%</div>
                          <div className="space-y-1">
                            {optimizedTeam.metrics.skills_coverage.map((detail, idx) => (
                              <div key={idx} className="flex justify-between text-[9px] border-b border-white/5 pb-1">
                                <span className="text-slate-400">{detail.skill}</span>
                                <span className="text-slate-200">Bridge Match: <strong className="text-cyan-400">{detail.contributed_by_skill || 'None'}</strong></span>
                              </div>
                            ))}
                          </div>
                        </div>
                  ...
                  {annealingStatus === 'idle' && (
                    <div className="text-slate-400 text-sm py-20 text-center">
                      Configure target skills, budget constraint, and click Solve to run Simulated Annealing team building.
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeSubTab === 'attrition' && (
            <motion.div key="attrition" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 text-left">
              {/* Left Employee list */}
              <div className="premium-card p-5 border border-white/5 bg-slate-950/40 backdrop-blur-md">
                <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300 mb-4 border-b border-white/5 pb-2">Employee Registry Attrition Hazard</h3>
                
                <div className="space-y-2 max-h-[460px] overflow-y-auto custom-scrollbar">
                  {attritionLoading ? (
                    <div className="text-xs text-slate-500 text-center py-8">Loading hazard computations...</div>
                  ) : (
                    attritionData.map((emp) => (
                      ...
                      >
                        <div>
                          <div className="font-bold text-white text-xs">{emp.full_name}</div>
                          <div className="text-[9px] text-slate-400 uppercase mt-0.5">{emp.role}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-xs font-black text-rose-300">x{emp.hazard_ratio}</div>
                          <div className="text-[8px] text-slate-500 uppercase mt-0.5">Hazard Ratio</div>
                        </div>
                      </button>
                    ))
                  )}
                ...
                {selectedAttritionEmp ? (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between border-b border-white/5 pb-3">
                      <div>
                        <div className="text-[9px] uppercase font-bold tracking-widest text-slate-500">Survival Hazard Breakdown & Simulation Sandbox</div>
                        <h3 className="text-xl font-extrabold text-white mt-1">{selectedAttritionEmp.full_name}</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
                      ...
                      <div className="space-y-4">
                        <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                          <div className="flex justify-between items-center mb-3">
                            <div className="text-[9px] uppercase font-bold tracking-widest text-slate-500">12-Month Survival Probability Curve</div>
                            <span className="text-xs font-bold text-indigo-400">End Projection Survival: {(simulatedSurvivalProb * 100).toFixed(1)}%</span>
                          </div>
                          ...
                          <div className="h-48 w-full relative border-b border-l border-white/5 bg-black/20 rounded">
                            <svg className="absolute inset-0 h-full w-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                              ...
                              <path
                                ...
                                stroke="none"
                                d={`M 0,${100 - (simulatedForecast[0]?.survival_probability * 100 || 100)} ` + 
                              ...
                              <polyline
                                ...
                                stroke="#6366f1"
                                strokeWidth="3"
                                points={simulatedForecast.map((f, i) => {
                                  const x = (i / 11) * 100
                                  const y = 100 - (f.survival_probability * 100)
                                  return `${x},${y}`
                                }).join(' ')}
                            ...
                            <div className="absolute left-2 top-2 text-[8px] text-slate-500 font-mono">100% S(t)</div>
                            <div className="absolute left-2 bottom-2 text-[8px] text-slate-500 font-mono">0% S(t)</div>
                            <div className="absolute right-2 bottom-2 text-[8px] text-slate-500 font-mono">12 Months Projection</div>
                          </div>
                        </div>

                        {/* Interactive Parameters Sandbox */}
                        <div className="rounded-xl border border-white/5 bg-slate-950/60 p-4">
                          <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-4 border-b border-white/5 pb-2">Flight Risk Mitigation Simulator</h4>
                          
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                              <div className="flex justify-between text-[9px] text-slate-400 mb-1">
                                <span>Morale Index</span>
                                <span className="text-emerald-400">{(moraleSlider * 100).toFixed(0)}%</span>
                              </div>
                              <input
                                type="range"
                                min="0.0"
                                ...
                                step="0.05"
                                value={moraleSlider}
                                onChange={(e) => setMoraleSlider(Number(e.target.value))}
                            ...
                            <div>
                              <div className="flex justify-between text-[9px] text-slate-400 mb-1">
                                ...
                                <span className="text-indigo-400">+{(salarySlider * 100).toFixed(0)}%</span>
                              </div>
                              <input
                                type="range"
                                min="0.0"
                            ...
                            <div>
                              <div className="flex justify-between text-[9px] text-slate-400 mb-1">
                                ...
                                <span className="text-amber-400">{workloadSlider} Nodes</span>
                              </div>
                              <input
                                type="range"
                                min="1"
                      ...
                      <div className="space-y-4">
                        <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                          <div className="text-[9px] uppercase tracking-widest text-slate-500 font-bold mb-1">Simulated Attrition Multiplier</div>
                          <div className="text-4xl font-black text-rose-400 font-mono">x{simulatedHazardRatio.toFixed(2)}</div>
                          <p className="text-[9px] text-slate-400 mt-2 leading-relaxed">
                            A hazard multiplier above 1.0 represents accelerated flight risk compared to average baseline probability.
                          </p>
                        </div>
                        
                        <div className="space-y-3">
                          <div className="text-[9px] uppercase tracking-widest text-slate-500 font-bold">Baseline Covariates (SHAP Explainability)</div>
                          {selectedAttritionEmp.covariates_explain.map((cov, idx) => (
                            <div key={idx} className="rounded-xl border border-white/5 bg-slate-950 p-3 flex flex-col justify-between">
                              <div className="flex items-center justify-between mb-1.5">
                                <span className="text-[10px] font-semibold text-slate-300">{cov.factor}</span>
                                <span className={`text-[9px] font-bold ${cov.impact_direction === 'risky' ? 'text-rose-400' : 'text-emerald-400'}`}>
                                  {cov.impact_direction === 'risky' ? '+' : ''}{cov.impact_percentage}% risk
                                </span>
                              </div>
                              <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${cov.impact_direction === 'risky' ? 'bg-rose-500' : 'bg-emerald-500'}`}
                                  style={{ width: `${Math.min(100, Math.abs(cov.impact_percentage))}%` }}
                                />
                              </div>
                            </div>
                          ))}
                  ...
                  <div className="text-slate-400 text-sm py-20 text-center">
                    Select an employee from the left panel to review attrition survival analytics.
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeSubTab === 'ona' && (
            <motion.div key="ona" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 text-left">
              {/* Left Graph Panel */}
              <div className="premium-card p-6 border border-white/5 bg-slate-950/20 flex flex-col justify-between relative overflow-hidden min-h-[460px]">
                <div>
                  <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-6">
                    <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">Corporate Collaboration Graph (Draggable Forces)</h3>
                    <span className="text-[10px] text-slate-500">PageRank Influence Leaders</span>
                  </div>
                  ...
                  >
                    {onaLoading ? (
                      <div className="text-slate-400 text-xs flex items-center gap-2">
                        <RefreshCw size={14} className="animate-spin" /> Resolving Brandes centrality paths...
                      </div>
                    ) : (
                      <>
                        {/* Links */}
                          ...
                          return (
                            <div
                              ...
                              className={`absolute rounded-full border transition-shadow duration-300 flex items-center justify-center group pointer-events-auto cursor-pointer select-none ${
                                isSelected ? 'bg-primary border-white shadow-[0_0_15px_rgba(45,212,191,0.8)] z-20 scale-105' : 'bg-slate-900 border-indigo-500/40 hover:border-cyan-300 z-10'
                              }`}
                            >
                ...
                {selectedOnaNode ? (
                  <>
                    <div className="border-b border-white/5 pb-3">
                      <div className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Corporate Node Centrality</div>
                      <h3 className="text-lg font-extrabold text-white mt-1">{selectedOnaNode.name}</h3>
                      <div className="text-[9px] text-slate-400 uppercase mt-0.5 tracking-wider">{selectedOnaNode.role}</div>
                    </div>

                    <div className="space-y-4">
                      {/* PageRank Card */}
                      <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                        <div className="text-[9px] uppercase tracking-[0.16em] text-slate-500 mb-1">PageRank Centrality (Influence)</div>
                        <div className="flex justify-between items-center">
                          <div className="text-3xl font-extrabold text-indigo-400">{(selectedOnaNode.influence_pagerank * 100).toFixed(0)}%</div>
                          {/* Radial indicator */}
                          <svg className="w-10 h-10 transform -rotate-90">
                            <circle cx="20" cy="20" r="16" stroke="rgba(255,255,255,0.05)" strokeWidth="3" fill="transparent" />
                            <circle cx="20" cy="20" r="16" stroke="#818cf8" strokeWidth="3" fill="transparent"
                              strokeDasharray={100}
                              strokeDashoffset={100 - (selectedOnaNode.influence_pagerank * 100)}
                            />
                          </svg>
                        </div>
                        <p className="text-[9px] text-slate-400 mt-2 leading-relaxed">
                          Measures overall connectivity and communication propagation strength. High PageRank nodes act as information multipliers.
                        </p>
                      </div>

                      {/* Betweenness Centrality Card */}
                      <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                        <div className="text-[9px] uppercase tracking-[0.16em] text-slate-500 mb-1">Betweenness Centrality (Bridges)</div>
                        <div className="flex justify-between items-center">
                          <div className="text-3xl font-extrabold text-cyan-400">{(selectedOnaNode.bridge_betweenness * 100).toFixed(0)}%</div>
                          {/* Radial indicator */}
                          <svg className="w-10 h-10 transform -rotate-90">
                            <circle cx="20" cy="20" r="16" stroke="rgba(255,255,255,0.05)" strokeWidth="3" fill="transparent" />
                            <circle cx="20" cy="20" r="16" stroke="#2dd4bf" strokeWidth="3" fill="transparent"
                              strokeDasharray={100}
                              strokeDashoffset={100 - (selectedOnaNode.bridge_betweenness * 100)}
                            />
                          </svg>
                        </div>
                        <p className="text-[9px] text-slate-400 mt-2 leading-relaxed">
                          Measures structural bridge strength across siloed departments. High betweenness employees prevent organization communication bottlenecks.
                        </p>
                      </div>
                    </div>
                  </>
                  ...
                  <div className="text-slate-400 text-xs py-16 text-center">
                    Select a collaboration node on the graph to analyze centralities.
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeSubTab === 'career-path' && (
            <motion.div key="career-path" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 text-left">
              {/* Left Selector */}
              <div className="premium-card p-5 border border-white/5 bg-slate-950/40 backdrop-blur-md flex flex-col justify-between">
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300 mb-4 border-b border-white/5 pb-2">Active Career Tracker</h3>
                  
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1.5 block">Select Employee Profile</label>
                      <select
                      ...
                      >
                        {careerEmployees.map(e => <option key={e.id} value={e.id}>{e.full_name}</option>)}
                      </select>
                    </div>

                    {careerPathData && (
                      <div className="rounded-xl border border-white/5 bg-slate-950 p-4">
                        <div className="text-[9px] uppercase tracking-widest text-slate-500 mb-1">Starting Node</div>
                        <div className="text-sm font-black text-white">{careerPathData.current_role}</div>
                        <div className="text-[9px] text-indigo-400 uppercase mt-0.5">Markov Chain State</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="premium-card p-6 border border-white/5 bg-slate-950/20">
                <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-6">
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">Markov Career Transition Horizon</h3>
                  <span className="text-[10px] text-slate-500">Calculated probabilities over 3-year timeline</span>
                </div>

                {careerLoading ? (
                  <div className="text-xs text-slate-500 text-center py-20">Matrix Multiplications under calculations...</div>
                ) : careerPathData ? (
                  <div className="space-y-8 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-0.5 before:bg-white/5 z-0">
                    {careerPathData.career_progression_markov.map((step, idx) => (
                      <div key={idx} className="relative pl-10 z-10">
                        ...
                        <div className="mb-2">
                          <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">{step.projected_time_horizon} Horizon</span>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {step.possibilities.map((pos, pIdx) => (
                            <div key={pIdx} className="rounded-xl border border-white/5 bg-slate-950 p-4 flex flex-col justify-between">
                              <div className="flex justify-between items-start mb-3">
                                <div>
                                  <div className="font-bold text-white text-sm">{pos.role}</div>
                                  <div className="text-[9px] text-slate-500 mt-0.5 uppercase">Target Transition State</div>
                                </div>
                                <span className="text-[10px] font-black text-cyan-300 bg-cyan-400/5 border border-cyan-400/10 px-2 py-0.5 rounded">
                                  {(pos.transition_probability * 100).toFixed(0)}% Prob.
                                </span>
                              </div>
                              ...
                              {pos.skill_gaps.length > 0 ? (
                                <div className="space-y-1.5 border-t border-white/5 pt-2.5">
                                  <div className="text-[8px] uppercase tracking-wider text-slate-500 font-bold mb-1">Required skill transitions</div>
                                  {pos.skill_gaps.map((gap, gIdx) => (
                                    <div key={gIdx} className="flex justify-between items-center text-[9px] border-b border-white/5 pb-1">
                                      <span className="text-slate-400">{gap.skill}</span>
                                      <span className={`font-mono text-[9px] ${gap.difficulty.includes('Easy') ? 'text-emerald-400' : gap.difficulty.includes('Medium') ? 'text-amber-400' : 'text-rose-400'}`}>
                                        {gap.difficulty}
                                      </span>
                                    </div>
                                  ))}
                  ...
                  <div className="text-slate-400 text-sm py-16 text-center">
                    Select an employee tracking profile to evaluate Markov Career progression path mapping.
                  </div>
                )}
              </div>
            </motion.div>
