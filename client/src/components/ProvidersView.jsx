...
import { 
  ShieldCheck, KeyRound, Cpu, Globe, RefreshCw, Box, 
  Check, AlertCircle, Play, Edit3, Network, Zap, ChevronRight,
  ArrowLeft, Plus, X, Layers, Activity, Settings2, Trash2,
  Code, Terminal, Copy, ExternalLink, Database, Calendar
} from 'lucide-react';
import Toast from './Toast';

const LLM_FAMILIES = [
  { id: 'anthropic', name: 'Anthropic', defaultUrl: 'https://api.anthropic.com/v1', placeholder: 'sk-ant-api03-...', standardModels: ['claude-3-5-sonnet-20241022', 'claude-3-haiku-20240307'] },
  { id: 'custom', name: 'OpenAI-Compatible', defaultUrl: 'https://api.openai.com/v1', placeholder: 'sk-proj-...', standardModels: ['gpt-4o-mini', 'gpt-4o'] },
  { id: 'google', name: 'Google', defaultUrl: 'https://generativelanguage.googleapis.com', placeholder: 'AIzaSy...', standardModels: ['gemini-1.5-flash', 'gemini-1.5-pro'] },
  { id: 'groq', name: 'Groq', defaultUrl: 'https://api.groq.com/openai/v1', placeholder: 'gsk_...', standardModels: ['llama-3.1-70b-versatile', 'llama-3.1-8b-instant'] },
  { id: 'lmstudio', name: 'LM Studio', defaultUrl: 'http://127.0.0.1:1234/v1', placeholder: 'http://127.0.0.1:1234/v1', standardModels: ['liquid/lfm2.5-1.2b', 'local-model'] },
  { id: 'ollama', name: 'Ollama', defaultUrl: 'http://127.0.0.1:11434', placeholder: 'http://127.0.0.1:11434', standardModels: ['llama3', 'mistral'] },
  { id: 'openai', name: 'OpenAI API', defaultUrl: 'https://api.openai.com/v1', placeholder: 'sk-proj-...', standardModels: ['gpt-4o-mini', 'gpt-4o', 'gpt-3.5-turbo'] },
  { id: 'opencode', name: 'OpenCode Zen', defaultUrl: 'https://opencode.ai/zen/v1', placeholder: 'oc-zen-...', standardModels: ['gpt-5.5', 'minimax-m2.5-free'] },
];
...
const SearchableModelSelect = ({ value, onChange, options, loading }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="font-mono truncate">{value !== 'Not Assigned' ? value : 'Select Active Model Brain'}</span>
        {loading ? (
          <RefreshCw size={14} className="text-cyan-400 animate-spin shrink-0" />
        ) : (
          <ChevronRight size={14} className={`text-slate-400 transition-transform duration-200 shrink-0 ${isOpen ? 'rotate-90' : ''}`} />
        )}
      </button>
        ...
        <div className="absolute z-50 w-full mt-1.5 rounded-xl border border-white/10 bg-[#08111e]/95 backdrop-blur-xl shadow-2xl p-2 flex flex-col max-h-60 min-h-0">
          <div className="relative mb-2 shrink-0">
            <input
              ...
              autoFocus
            />
            <Globe className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500" size={13} />
            {search && (
              <button
                type="button"
                onClick={() => setSearch('')}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto space-y-1 custom-scrollbar">
            ...
            >
              ...
              {value === 'Not Assigned' && <Check size={11} className="stroke-[3]" />}
            </button>
...
const ProvidersView = () => {
      ...
      LLM_FAMILIES.forEach(fam => {
        if (parsed[fam.id] && (parsed[fam.id].key || parsed[fam.id].endpoint)) {
          list.push({
            ...
            url: parsed[fam.id].base_url || parsed[fam.id].endpoint || fam.defaultUrl,
            selectedModel: parsed[fam.id].selectedModel || fam.standardModels[0],
  ...
  const computeHmacSignature = async (secret, body) => {
    try {
      ...
      const signatureBuffer = await window.crypto.subtle.sign('HMAC', cryptoKey, encoder.encode(body));
  ...
  const fetchIntegrationLogs = async () => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${apiBase}/api/v1/integrations/logs?limit=20`, {
        headers: getAuthHeaders(),
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchWebhookEvents = async () => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${apiBase}/api/v1/integrations/events?limit=20`, {
        headers: getAuthHeaders(),
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleGenerateToken = async () => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${apiBase}/api/v1/integrations/token?name=${encodeURIComponent(newTokenName)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      if (response.ok) {
  ...
  const handleRevokeToken = async (tokenId) => {
    try {
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${apiBase}/api/v1/integrations/token/${tokenId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
      if (response.ok) {
  ...
  const handleSimulateIngest = async (type) => {
    ...
    try {
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const activeToken = generatedKey || (tokens.length > 0 ? tokens[0].api_key : null);
      if (!activeToken) {
        showToast('No integration token is available. Generate one while logged in as admin.', 'error');
        return;
      }
      ...
      if (!generatedKey && tokens.length === 0 && authHeaders.Authorization) {
        const genResp = await fetch(`${apiBase}/api/v1/integrations/token?name=Demo-Simulator`, {
          method: 'POST',
          headers: authHeaders,
        });
        if (genResp.ok) {
      ...
      if (response.ok) {
        const data = await response.json();
        showToast(`Simulated ${type.toUpperCase()} Ingestion Event Succeeded!`, 'success');
      } else {
        const errData = await response.json();
        showToast(`Simulation failed: ${errData.detail || errData.message || 'check parameters'}`, 'error');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSelectFamily = (family) => {
    ...
    setFormData({
      authProtocol: family.id === 'lmstudio' || family.id === 'ollama' ? 'None' : 'API Key',
  ...
  const discoverModels = async (currentUrl = formData.url, currentToken = formData.token) => {
    ...
    try {
      ...
      const apiBase = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      const response = await fetch(`${apiBase}/api/v1/chat/providers/discover`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (data.status === 'success' && data.models && data.models.length > 0) {
          ...
          if (formData.selectedModel === 'Not Assigned' || !data.models.includes(formData.selectedModel)) {
            setFormData(prev => ({ ...prev, selectedModel: data.models[0] }));
          }
          showToast(`Discovered ${data.models.length} active models!`, 'success');
        } else {
          setAvailableModels(selectedConnection.standardModels);
          showToast('Live discovery failed, loaded pre-defined models.', 'info');
        }
      } else {
        setAvailableModels(selectedConnection.standardModels);
      }
  ...
  useEffect(() => {
    ...
    const isLocalOrCustom = selectedConnection.id === 'lmstudio' || selectedConnection.id === 'ollama' || selectedConnection.id === 'custom' || formData.authProtocol === 'None';
    const canDiscover = isLocalOrCustom ? (formData.url && formData.url.length > 5) : (formData.token && formData.token.length > 5);
  ...
  const testPing = async () => {
    ...
    const payload = {
      ...
      model: formData.selectedModel !== 'Not Assigned' ? formData.selectedModel : selectedConnection.standardModels[0],
    };
    ...
    try {
      ...
      setPingMessage(`Ping failed: ${err.message || 'Server host is unreachable'}`);
      showToast('API Host offline', 'error');
    } finally {
      setPinging(false);
    }
  ...
  const handleLinkFamily = () => {
    ...
    // Save to states
    const isAlreadyLinked = linkedConnections.some(c => c.id === selectedConnection.id);
    const updated = {
      ...selectedConnection,
      key: formData.token,
      url: formData.url,
      selectedModel: formData.selectedModel !== 'Not Assigned' ? formData.selectedModel : selectedConnection.standardModels[0],
      isActive: true
    };

    let newConnections = [];
    if (isAlreadyLinked) {
      newConnections = linkedConnections.map(c => c.id === selectedConnection.id ? updated : { ...c, isActive: false });
    } else {
      newConnections = [...linkedConnections.map(c => ({ ...c, isActive: false })), updated];
    }
    ...
    localStorage.setItem('AURELIUS_PROVIDERS_CONFIG', JSON.stringify(savedConfig));
  ...
  const handleToggleActive = (connId) => {
    ...
    setLinkedConnections(prev =>
      prev.map(c => c.id === connId ? { ...c, isActive: true } : { ...c, isActive: false })
    );
        ...
        {viewMode === 'list' ? (
          ...
          >
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-400">Manage your system credentials, preferences, and external connectors.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-white group-hover:text-cyan-300 transition-colors">Providers</h3>
              ...
              <div className="premium-card p-6 flex items-start gap-4 border border-white/5 opacity-55 hover:opacity-75 transition-opacity text-left">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-300">Preferences</h3>
                  <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                    Adjust interface densities, workspace theme tokens, and logging.
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-b border-white/5 pb-4">
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-2xl font-black text-white tracking-tight leading-none uppercase">Providers</h1>
            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-4">
                  ...
                  {inventoryMode === 'list' ? (
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Inventory</span>
                        <button
                      ...
                      <div className="flex-1 min-h-0 overflow-y-auto space-y-2.5">
                        {linkedConnections.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-10 border border-dashed border-white/5 rounded-xl text-slate-500 p-4 text-center">
                            <span className="text-xs font-mono mb-1">No active connections.</span>
                            <span className="text-[10px]">Click + to add a provider family.</span>
                          </div>
                        ) : (
                              ...
                              >
                                <div className="flex items-center gap-2.5 min-w-0">
                                  ...
                                  <div className="truncate">
                                    ...
                                    <p className="text-[9px] text-slate-400 font-mono truncate">{conn.selectedModel}</p>
                                  </div>
                                </div>
                                <button
                                  onClick={(e) => unlinkConnection(conn.id, e)}
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Select Family</span>
                        <button
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="text-[9px] uppercase tracking-wider text-slate-400 font-bold font-mono">
                                {family.id === 'lmstudio' || family.id === 'ollama' ? 'Local' : 'Hosted API'}
                              </div>
                            </div>
                          </button>
                        ))}
              ...
              <div className={`${activeCategory === 'LLM' ? 'lg:col-span-3' : 'lg:col-span-4'} border border-white/5 bg-[#081220]/60 rounded-2xl p-6 flex flex-col min-h-0`}>
                <AnimatePresence mode="wait">
                  {activeCategory === 'NAVIGATOR' ? (
                    ...
                    >
                      <div className="border-b border-white/5 pb-3 mb-4 flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase tracking-widest text-slate-400 font-bold font-mono">System Navigator</span>
                          <h2 className="text-xl font-black text-white leading-none uppercase mt-0.5">Applied Foundation Models</h2>
                          <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wide">Verify active cognitive runtimes. Only one model can be active at a time.</p>
                        </div>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                        {linkedConnections.length === 0 ? (
                          <div className="flex-1 flex flex-col items-center justify-center text-center py-16">
                            ...
                            <h3 className="text-lg font-bold text-white mb-1 uppercase">No Configured Models</h3>
                            <p className="text-xs text-slate-400 max-w-sm leading-relaxed mb-4">
                              Go to the LLM tab and click "+" under the Inventory sidebar to set up your first runtime brain.
                            </p>
                            <button
                                  ...
                                  <div className="flex items-start gap-3.5 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <div className="flex items-center gap-2">
                                        <h4 className="text-sm font-extrabold text-white">{conn.name}</h4>
                                        <span className={`w-2 h-2 rounded-full ${isCurrentlyActive ? 'bg-cyan-400 animate-pulse' : 'bg-slate-600'}`} />
                                      </div>
                                      <div className="font-mono text-xs text-cyan-200 mt-0.5 truncate">{conn.selectedModel}</div>
                                      <div className="font-mono text-[9px] text-slate-400 mt-1 truncate">{conn.url}</div>
                                    </div>
                                  </div>
                        ...
                        {linkedConnections.length > 0 && (
                          <div className="mt-6 rounded-2xl border border-white/5 bg-slate-950/40 p-4 flex gap-3 items-start">
                            ...
                            <div>
                              <h5 className="text-xs font-extrabold text-white uppercase tracking-wider mb-0.5">End-to-End Credential Privacy</h5>
                              <p className="text-[10px] text-slate-400 leading-relaxed">
                                Aurelius enforces strict zero-knowledge security protocols. All authorization tokens, keys, and endpoint credentials reside securely inside your local browser client memory. They are never transmitted, cached, or written to our backend database under any condition.
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    >
                      <div className="border-b border-white/5 pb-3 mb-4 flex items-center justify-between">
                        <div>
                          <span className="text-[9px] uppercase tracking-widest text-slate-400 font-bold font-mono">B2B Middleware Console</span>
                          <h2 className="text-xl font-black text-white leading-none uppercase mt-0.5">Enterprise API Ingestions & SDK</h2>
                          <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wide">Secure live synchronization for Jira, Slack, Workday APIs, and custom B2B SDK integrations.</p>
                        </div>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-6 pr-1 custom-scrollbar">
                        ...
                        <div className="premium-card p-5 border border-white/5 bg-[#0a1222]/40 rounded-2xl">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Integrations API Key Registry</h3>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                              ...
                              >
                                <option value={employeeEmail}>{employeeEmail} (Active Dev)</option>
                                <option value="silas.vance@aurelius.io">silas.vance@aurelius.io (Seed lead)</option>
                                <option value="clara.sutton@aurelius.io">clara.sutton@aurelius.io (Senior dev)</option>
                              </select>
                            </div>
                          </div>
                          ...
                          {generatedKey && (
                            ...
                            >
                              <div className="min-w-0">
                                <span className="text-[8px] uppercase tracking-widest text-emerald-400 font-bold block font-mono">New Secret Key Generated</span>
                                <div className="text-xs font-mono text-white truncate select-all font-bold mt-0.5">{generatedKey}</div>
                                <span className="text-[8px] text-slate-400 mt-1 block">Copy this token securely. It will not be shown again!</span>
                              </div>
                              <button
                                type="button"
                                onClick={() => {
                            ...
                            <table className="w-full text-left font-mono text-[10px] text-slate-400 border border-white/5 rounded-xl overflow-hidden">
                              <thead>
                                <tr className="bg-slate-950/60 text-slate-400 border-b border-white/5">
                                  <th className="p-3 text-[9px] uppercase tracking-widest font-bold font-mono">Credential Name</th>
                                  <th className="p-3 text-[9px] uppercase tracking-widest font-bold font-mono">Created At</th>
                                  <th className="p-3 text-[9px] uppercase tracking-widest font-bold font-mono">Expires At</th>
                                  <th className="p-3 text-[9px] uppercase tracking-widest font-bold font-mono">Status</th>
                                ...
                                {loadingTokens ? (
                                  <tr>
                                    <td colSpan={5} className="p-6 text-center text-slate-500">
                                      <RefreshCw size={14} className="animate-spin inline mr-1 text-cyan-400" />
                                  ...
                                  <tr>
                                    <td colSpan={5} className="p-6 text-center text-slate-500">
                                      No B2B Ingestion Credentials registered yet. Click Generate Token above.
                                    </td>
                                  </tr>
                                ) : (
                                  tokens.map((token) => (
                                    <tr key={token.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                      ...
                                      <td className="p-3">
                                        <span className={`px-2 py-0.5 rounded text-[8px] uppercase tracking-wider font-extrabold ${
                                          token.status === 'active' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-800 text-slate-500'
                                        }`}>
                                          {token.status}
                                        </span>
                                      </td>
                        ...
                        <div className="premium-card p-5 border border-white/5 bg-[#0a1222]/40 rounded-2xl">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Secure Ingest Webhooks (Jira, Slack, Workday)</h3>
                          </div>

                          <p className="text-[10px] text-slate-400 leading-relaxed mb-4">
                            This ingestion pipeline uses header-based protection: <span className="font-mono text-slate-100">X-API-Key</span>, <span className="font-mono text-slate-100">X-Signature</span>, and optional <span className="font-mono text-slate-100">Idempotency-Key</span>. URLs shown below are endpoint targets only.
                          </p>
                          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                            ...
                            <div className="p-4 rounded-xl border border-white/5 bg-slate-950/40 flex flex-col justify-between h-full">
                              ...
                              <div className="space-y-2 mt-auto">
                                <div className="p-2 bg-black/60 rounded-lg text-[9px] font-mono text-slate-300 select-all truncate">
                                  {`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/jira`}
                                </div>
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => {
                                      navigator.clipboard.writeText(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/jira`);
                                  ...
                                  >
                                    {simulatingWebhook === 'jira' ? <RefreshCw size={11} className="animate-spin" /> : <Play size={11} />}
                                    Simulate Event
                                  </button>
                                </div>
                              </div>
                            ...
                            <div className="p-4 rounded-xl border border-white/5 bg-slate-950/40 flex flex-col justify-between h-full">
                              ...
                              <div className="space-y-2 mt-auto">
                                <div className="p-2 bg-black/60 rounded-lg text-[9px] font-mono text-slate-300 select-all truncate">
                                  {`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/slack`}
                                </div>
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => {
                                      navigator.clipboard.writeText(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/slack`);
                                  ...
                                  >
                                    {simulatingWebhook === 'slack' ? <RefreshCw size={11} className="animate-spin" /> : <Play size={11} />}
                                    Simulate Event
                                  </button>
                                </div>
                              </div>
                            ...
                            <div className="p-4 rounded-xl border border-white/5 bg-slate-950/40 flex flex-col justify-between h-full">
                              ...
                              <div className="space-y-2 mt-auto">
                                <div className="p-2 bg-black/60 rounded-lg text-[9px] font-mono text-slate-300 select-all truncate">
                                  {`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/workday`}
                                </div>
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => {
                                      navigator.clipboard.writeText(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/v1/integrations/workday`);
                                  ...
                                  >
                                    {simulatingWebhook === 'workday' ? <RefreshCw size={11} className="animate-spin" /> : <Play size={11} />}
                                    Simulate Event
                                  </button>
                                </div>
                              </div>
                        ...
                        <div className="premium-card p-5 border border-white/5 bg-[#0a1222]/40 rounded-2xl text-left">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Integration Delivery Dashboard</h3>
                          </div>
                          ...
                          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
                            <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-4">
                              <div className="flex items-center justify-between mb-3">
                                <div>
                                  <p className="text-[9px] uppercase tracking-wider text-slate-400 font-bold">Recent Logs</p>
                              ...
                              <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                                {integrationLogs.length === 0 ? (
                                  <div className="text-[10px] text-slate-500">No logs available for this tenant or admin session.</div>
                                ) : (
                                  integrationLogs.slice(0, 8).map((entry) => (
                                    <div key={entry.id} className="rounded-xl border border-white/10 bg-[#08111e]/90 p-3 text-[10px] text-slate-300">
                                      ...
                                      <div className="text-[9px] text-slate-500 mt-2 flex items-center justify-between">
                                        ...
                                        <span className={`px-2 py-0.5 rounded ${entry.status === 'success' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-rose-500/10 text-rose-300'}`}>{entry.status}</span>
                                      </div>
                                    </div>
                                  ))
                                )}
                            ...
                            <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-4">
                              <div className="flex items-center justify-between mb-3">
                                <div>
                                  <p className="text-[9px] uppercase tracking-wider text-slate-400 font-bold">Webhook Events</p>
                              ...
                              <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                                {webhookEvents.length === 0 ? (
                                  <div className="text-[10px] text-slate-500">No webhook delivery events found for this tenant.</div>
                                ) : (
                                  webhookEvents.slice(0, 8).map((event) => (
                                    <div key={event.id} className="rounded-xl border border-white/10 bg-[#08111e]/90 p-3 text-[10px] text-slate-300">
                                      <div className="font-bold text-slate-100 truncate">{event.integration_name.toUpperCase()} • {event.endpoint}</div>
                                      <div className="text-slate-400 truncate">Idempotency: {event.idempotency_key || 'none'}</div>
                                      <div className="text-[9px] text-slate-500 mt-2 flex items-center justify-between">
                        ...
                        <div className="premium-card p-5 border border-white/5 bg-[#0a1222]/40 rounded-2xl text-left">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Aurelius Developer SDK Setup Guide</h3>
                          </div>

                          <p className="text-[10px] text-slate-400 leading-relaxed mb-4">
                            Integrate your enterprise backend pipeline directly to feed Aurelius. You can import our library into B2C/B2B services to push telemetry events instantly.
                          </p>
                          ...
                          <div className="rounded-xl border border-white/10 bg-slate-950 overflow-hidden flex flex-col font-mono text-[10px] text-slate-300">
                            ...
                            <div className="bg-slate-900 px-4 py-2 text-[9px] uppercase tracking-wider text-slate-400 font-bold border-b border-white/5 flex items-center justify-between">
                              ...
                              <span className="text-cyan-400">Node / PyPI Ready</span>
                            </div>
...
await aurelius.pushSentimentMetric({
  ...
  message_count: 142
                    ...
                    >
                      ...
                      <h2 className="text-xl font-bold text-white mb-2 leading-none uppercase">System Navigator</h2>
                      <p className="text-xs text-slate-400 max-w-sm leading-relaxed">
                        Select a provider from the inventory to adjust its parameters or test connectivity.
                      </p>
                    </motion.div>
                  ) : (
                    ...
                    >
                      <div className="border-b border-white/5 pb-3 mb-4">
                        <span className="text-[9px] uppercase tracking-widest text-slate-400 font-bold font-mono">Chat Setup</span>
                        <h2 className="text-2xl font-black text-white leading-none uppercase mt-0.5">{selectedConnection.name}</h2>
                        <p className="text-xs text-slate-400 mt-1">Connect this chat provider, enter the API key, then link it.</p>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setFormData({ ...formData, authProtocol: e.target.value })}
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              />
                              <Globe className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} />
                            </div>
                          </div>
                        </div>

                         {/* Security Token / Key Input */}
                         {selectedConnection.id !== 'lmstudio' && selectedConnection.id !== 'ollama' ? (
                           <div>
                             <div className="flex items-center justify-between mb-1.5">
                               <label className="block text-[9px] uppercase tracking-widest text-slate-400 font-bold font-mono">
                                 Security Token / Secret
                               </label>
                               <span className="text-[8px] uppercase tracking-wider text-slate-400 font-bold">API Key / Token</span>
                             </div>
                             <div className="relative">
                               <input
                                 type="password"
                               ...
                               />
                               <KeyRound className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} />
                             </div>
                             <div className="mt-1 text-[9px] text-slate-400 flex items-center gap-1.5">
                               <ShieldCheck size={11} className="text-cyan-300 shrink-0" />
                           ...
                           <div className="rounded-xl border border-cyan-500/10 bg-cyan-950/10 p-4">
                             <div className="flex items-center gap-2 mb-1">
                               <ShieldCheck className="text-cyan-300 shrink-0" size={14} />
                               <span className="text-xs font-extrabold text-white uppercase tracking-wider">No Authentication Required</span>
                             </div>
                             <p className="text-[10px] text-slate-400 leading-relaxed">
                               This is a local provider runtime. No API authorization key or token is required. Connection and discovery are initialized purely over your local loopback port URL.
                             </p>
                           </div>
                         )}
                         ...
                         <div>
                            <div className="flex items-center justify-between mb-1.5">
                              ...
                              <button
                                type="button"
                                onClick={() => discoverModels(formData.url, formData.token)}
                              ...
                              >
                                <RefreshCw size={8} className={loadingModels ? 'animate-spin' : ''} />
                                Refresh Discovery
                              </button>
                            </div>
                            <SearchableModelSelect
                              value={formData.selectedModel}
                              onChange={(val) => setFormData({ ...formData, selectedModel: val })}
                         ...
                         <div className="rounded-xl border border-white/5 bg-slate-950/40 p-4">
                           <div className="flex items-center justify-between mb-2">
                             <span className="text-[9px] uppercase tracking-widest text-slate-400 font-bold font-mono">Model Discovery</span>
                             {selectedConnection.id === 'lmstudio' || selectedConnection.id === 'ollama' ? (
                               ...
                               <span className={`inline-flex items-center px-2 py-0.5 rounded text-[8px] uppercase tracking-wider font-extrabold ${
                                 formData.token ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'
                               }`}>
                                 {formData.token ? 'Key Detected' : 'Waiting for Token'}
                               </span>
                             )}
                           </div>
                           <p className="text-[10px] text-slate-400 leading-relaxed">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          ...
                          <div className="rounded-xl border border-white/5 bg-white/5 p-3.5">
                            <div className="text-[8px] uppercase tracking-widest text-slate-400 mb-1 font-bold font-mono">Current Target</div>
                            <div className="text-xs font-bold text-white truncate">
                              {formData.selectedModel !== 'Not Assigned' ? formData.selectedModel : 'No model selected'}
                            </div>
                            <div className="text-[9px] text-slate-400 mt-0.5">Hosted chat runtime</div>
                          </div>

                          {/* Card 2: RUNTIME URL */}
                          <div className="rounded-xl border border-white/5 bg-white/5 p-3.5">
                            <div className="text-[8px] uppercase tracking-widest text-slate-400 mb-1 font-bold font-mono">Runtime URL</div>
                            <div className="text-xs font-mono text-white truncate">
                              {formData.url || 'Not set'}
                            </div>
                            <div className="text-[9px] text-slate-400 mt-0.5">Active base gateway</div>
                          </div>

                          {/* Card 3: STATUS */}
                          <div className={`rounded-xl border p-3.5 ${
                            pingStatus === 'connected' ? 'border-emerald-500/20 bg-emerald-500/5' :
                            pingStatus === 'error' || pingStatus === 'offline' ? 'border-rose-500/20 bg-rose-500/5' :
                            'border-white/5 bg-white/5'
                          }`}>
                            <div className="text-[8px] uppercase tracking-widest text-slate-400 mb-1 font-bold font-mono">Status</div>
                            <div className={`text-xs font-bold truncate ${
                              pingStatus === 'connected' ? 'text-emerald-300' :
                              pingStatus === 'error' || pingStatus === 'offline' ? 'text-rose-300' :
                              'text-slate-300'
                            }`}>
                              {pingStatus === 'connected' ? 'Connected' :
                               pingStatus === 'error' || pingStatus === 'offline' ? 'Offline' :
                               'Not connected'}
                            </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between shrink-0">
                        <div className="flex gap-4">
                          ...
                          >
                            {pinging ? <RefreshCw size={11} className="animate-spin" /> : <Activity size={11} />}
                            Test Ping
                          </button>

                          <button
                            onClick={() => {
                              showToast('Configuration ready. Lock active family to build pipeline.', 'info');
                            }}
