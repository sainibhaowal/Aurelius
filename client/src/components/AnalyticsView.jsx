...
const AnalyticsView = () => {
  ...
  useEffect(() => {
    const stream = new EventSource(`${API_BASE}/api/v1/ai/analytics/stream`);
  ...
  return (
    <div className="w-full pb-20">
      <header className="mb-8">
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 text-white">Organizational Analytics</h1>
        <p className="text-slate-400 text-sm md:text-base leading-relaxed max-w-3xl">Deep-dive telemetry into workforce distribution and health clusters.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        <MetricCard title="Total Workforce" value={stats.total} delta="Live SSE stream" color="primary" />
        <MetricCard title="At-Risk Employees" value={stats.atRisk} delta={`${stats.atRiskPct}% of workforce`} color="risk" />
        <MetricCard title="Active Departments" value={stats.depts.length} delta="Current org structure" color="accent" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass-card p-10">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-xl font-bold uppercase tracking-tight">Workforce Distribution</h3>
          </div>
            ...
            {stats.depts.map((dept) => (
              <div key={dept.name} className="space-y-2">
                <div className="flex justify-between text-sm font-bold">
                  <span className="text-white/60 uppercase tracking-widest">{dept.name}</span>
                  <span className="text-white">{dept.count} Members</span>
                </div>
                <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${stats.total > 0 ? (dept.count / stats.total) * 100 : 0}%` }}
        ...
        <div className="glass-card p-10 flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-xl font-bold uppercase tracking-tight">Predictive Risk Vector</h3>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center relative">
            ...
            <div className="text-center z-10">
              <div className="text-6xl font-black neon-text">{stats.atRiskPct.toFixed(1)}%</div>
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mt-2">{stats.riskLevel} Risk Coefficient</div>
            </div>
          </div>
...
const MetricCard = ({ title, value, delta, color }) => (
  <div className="glass-card p-8 border-l-4 border-l-white/5 hover:border-l-primary transition-all duration-500">
    <div className="flex justify-between items-start mb-4">
      ...
      <TrendingUp size={16} className={color === 'risk' ? 'text-rose-400' : color === 'accent' ? 'text-emerald-300' : 'text-cyan-300'} />
    </div>
    <div className="text-4xl font-black mb-1">{value}</div>
    <div className={`text-xs font-bold ${color === 'risk' ? 'text-rose-300' : color === 'accent' ? 'text-emerald-300' : 'text-cyan-300'}`}>
      {delta}
    </div>
  </div>
);
