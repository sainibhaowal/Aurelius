    ...
    >
      <div className="flex items-start gap-3.5 relative z-10 h-full">
        ...
        <div className="w-10 h-10 md:w-11 md:h-11 rounded-lg bg-gradient-to-br from-indigo-500/10 to-cyan-500/10 flex items-center justify-center border border-white/10 shrink-0 shadow-inner group-hover:from-indigo-500/20 transition-all duration-500">
          <User className="text-white/20 group-hover:text-cyan-400 transition-colors" size={18} />
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col h-full">
          <div className="flex items-start justify-between gap-3 mb-1.5">
            <div className="min-w-0 flex-1">
              <h3 className="text-sm md:text-base font-extrabold text-white tracking-tight leading-snug truncate mb-1" title={talent.full_name}>
                {talent.full_name}
              </h3>
              <p className="text-[9px] text-white/30 font-bold uppercase tracking-[0.15em] flex items-center gap-1.5 min-w-0">
                ...
                <span className="truncate flex-shrink" title={talent.role}>{talent.role}</span>
                <span className="text-white/10 shrink-0">-</span> 
                <span className="truncate flex-shrink" title={talent.department}>{talent.department}</span>
              </p>
            </div>
            ...
            >
              {talent.is_at_risk ? 'At-Risk' : 'Optimal'}
            </Badge>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 my-2.5">
            {talent.skills?.slice(0, 3).map((skill, i) => (
              ...
              >
                {skill.name} <span className="text-cyan-400 ml-1.5 font-black">L{skill.level}</span>
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/5 mt-auto gap-3">
            <div className="flex gap-4 sm:gap-5">
              <div className="space-y-0.5">
                <p className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-500">Sentiment</p>
                <p className="text-xs font-black text-white">{talent.sentiment_score}</p>
              </div>
              <div className="space-y-0.5">
                <p className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-500">Risk Prob</p>
                <p className="text-xs font-black text-white">
                  {talent.retention_prob ? (talent.retention_prob * 100).toFixed(0) : '0'}%
                </p>
              </div>
            </div>
