...
const Badge = ({
  ...
  const baseBadge = 'inline-flex items-center px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.15em] border whitespace-nowrap select-none';
  ...
  const variants = {
    ...
    glow: {
      primary: 'bg-indigo-500/5 border-indigo-500/20 text-indigo-300 shadow-[0_0_12px_rgba(99,102,241,0.06)]',
      secondary: 'bg-white/2 border-white/8 text-slate-300 shadow-sm',
      success: 'bg-emerald-500/5 border-emerald-500/20 text-emerald-300 shadow-[0_0_12px_rgba(16,185,129,0.06)]',
      warning: 'bg-amber-500/5 border-amber-500/20 text-amber-300 shadow-[0_0_12px_rgba(245,158,11,0.06)]',
      danger: 'bg-rose-500/5 border-rose-500/20 text-rose-300 shadow-[0_0_12px_rgba(244,63,94,0.06)]',
  ...
  return (
    ...
    >
      ...
      {variant === 'glow' && (
        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 shrink-0 bg-current`} />
      )}
      {children}
    </span>
  );
