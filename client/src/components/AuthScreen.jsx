...
const AuthScreen = () => {
  ...
  const switchToLogin = (email = '') => {
    setMode('login')
    setLoginForm((prev) => ({ ...prev, email: email || prev.email, password: '' }))
  }
  ...
  return (
    <div
      className="min-h-screen text-slate-100 relative overflow-hidden flex"
    ...
    >
      ...
      <aside
        ...
        style={{ background: 'rgba(4,10,18,0.75)', borderRight: '1px solid rgba(255,255,255,0.05)' }}
      >
        {/* Logo */}
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          >
            ...
            <h1 className="text-[3.2rem] font-bold leading-[1.08] tracking-tight text-white mb-7">
              Intelligence begins<br />
              with{' '}
              <span style={{ color: '#67e8f9' }}>access.</span>
            </h1>

            <p className="text-slate-400 text-[15px] leading-7 max-w-sm">
              Register once, then sign in to your workforce analytics platform — talent data,
              AI insights, and enterprise connectors unified.
            </p>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {mode === 'register' ? (
                ...
                >
                  ...
                  {registerError && <StatusMsg tone="error" text={registerError} />}
                  {registerSuccess && <StatusMsg tone="success" text={registerSuccess} />}
...
const LineField = ({ label, type = 'text', value, onChange, placeholder }) => (
  <div>
    ...
    <input
      ...
      className="w-full text-sm text-white outline-none transition-colors pb-2.5"
      style={{
      ...
      onFocus={(e) => { e.currentTarget.style.borderBottomColor = 'rgba(103,232,249,0.5)' }}
...
const SubmitBtn = ({ loading, accent, children }) => {
  const bg = accent === 'cyan'
    ? { base: '#67e8f9', hover: '#a5f3fc' }
    : { base: '#6ee7b7', hover: '#a7f3d0' }
  ...
  return (
    <button
      ...
      style={{ background: bg.base, color: '#0a1628' }}
      onMouseEnter={(e) => { if (!loading) e.currentTarget.style.background = bg.hover }}
