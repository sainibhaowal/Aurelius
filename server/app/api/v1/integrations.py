...
async def verify_payload_signature(
    ...
    computed_signature = hmac.new(raw_key.encode("utf-8"), body_bytes, hashlib.sha256).hexdigest()
...
def get_idempotency_key(idempotency_key: Optional[str] = Header(None, alias="Idempotency-Key")) -> Optional[str]:
...
def create_ingestion_token(
    name: str = Query(..., description="A friendly name for this integration credential, e.g. Jira-Webhook"),
...
def list_active_tokens(
    ...
    tokens = session.exec(
        select(IntegrationApiKeyTable).where(IntegrationApiKeyTable.tenant_id == tenant_id)
    ).all()
    return [
        {
            "id": str(t.id),
...
def revoke_token(
    ...
    db_key = session.exec(select(IntegrationApiKeyTable).where(IntegrationApiKeyTable.id == token_id)).first()
    if not db_key:
...
def rotate_token(
    ...
    db_key = session.exec(select(IntegrationApiKeyTable).where(IntegrationApiKeyTable.id == token_id, IntegrationApiKeyTable.tenant_id == tenant_id)).first()
    if not db_key:
        raise HTTPException(status_code=404, detail="Token credential not found")
...
def ingest_workday_hris(
    ...
    if not employee_data or not employee_data.get("email"):
        update_webhook_event_status(session, event, success=False, error_message="Missing employee contact or email")
        raise HTTPException(status_code=400, detail="Missing required 'employee' details or 'email' address.")
    ...
    emp = session.exec(select(EmployeeTable).where(EmployeeTable.email == email)).first()
    summary = ""
    ...
    if action == "hire" and not emp:
        ...
        existing_skills = {
            s.name.lower()
            for s in session.exec(select(SkillTable).where(SkillTable.employee_id == emp.id)).all()
            if s.name
        }
        for s in skills:
            s_name = s.get("name")
            if s_name and s_name.lower() not in existing_skills:
                ...
                session.add(skill_obj)
        session.commit()
        summary = f"Workday Sync: Updated employee profile details for {full_name} ({email})."
...
def ingest_slack_sentiment(
    ...
    if not email or sentiment is None:
        update_webhook_event_status(session, event, success=False, error_message="Missing required email or sentiment_score")
        raise HTTPException(status_code=400, detail="Missing required parameters: 'email' and 'sentiment_score'")

    email = email.strip().lower()
    emp = session.exec(select(EmployeeTable).where(EmployeeTable.email == email)).first()
    if not emp:
        update_webhook_event_status(session, event, success=False, error_message=f"Employee not found: {email}")
        raise HTTPException(status_code=404, detail=f"Employee with email {email} not found in directory.")
    ...
    return {"status": "success", "message": summary, "current_hazard_factor": emp.sentiment_score}
...
def ingest_jira_collaboration(
    ...
    if not reporter_email or not assignee_email:
        update_webhook_event_status(session, event, success=False, error_message="Missing reporter or assignee email")
        raise HTTPException(status_code=400, detail="Both 'reporter_email' and 'assignee_email' are required.")

    reporter_email = reporter_email.strip().lower()
    assignee_email = assignee_email.strip().lower()

    rep = session.exec(select(EmployeeTable).where(EmployeeTable.email == reporter_email)).first()
    asg = session.exec(select(EmployeeTable).where(EmployeeTable.email == assignee_email)).first()

    if not rep or not asg:
        update_webhook_event_status(session, event, success=False, error_message="Reporter or assignee not found in directory")
        raise HTTPException(
            status_code=400,
    ...
    return {"status": "success", "message": "Jira telemetry ingested successfully", "collaboration": summary}
