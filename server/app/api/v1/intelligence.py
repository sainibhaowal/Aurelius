...
SKILL_GRAPH: Dict[str, List[Tuple[str, float]]] = {
    # Frontend Ecosystem
    "React": [("JavaScript", 0.05), ("Next.js", 0.1), ("TypeScript", 0.15), ("Frontend", 0.2)],
    ...
    "Deep Learning": [("Machine Learning", 0.1), ("PyTorch", 0.15), ("TensorFlow", 0.15)],
...
def calculate_skill_match(candidate_skills: List[Dict[str, Any]], target_skills: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    for target in target_skills:
        ...
        matches.append({
            ...
            "semantic_distance": round(best_path_distance, 3) if best_path_distance < 99 else None,
            "match_confidence": round(best_match_score, 3),
            "status": "Perfect" if best_match_score >= 0.95 else
                      "Highly Transferable" if best_match_score >= 0.7 else
                      "Trainable Gap" if best_match_score >= 0.4 else "Missing Node"
        })
        
    overall_match = round(total_score / len(target_skills), 3) if target_skills else 0.0
    return {
        "overall_compatibility": overall_match,
        "detailed_matches": matches
    }

@router.post("/skill-match")
def match_skills(payload: Dict[str, Any], session: Session = Depends(get_session), current_user: TokenData = Depends(get_current_user)):
    ...
    employees = [
        emp for emp in filter_real_records(session.exec(select(EmployeeTable)).all())
        if emp is not None and getattr(emp, "id", None) is not None and getattr(emp, "join_date", None) is not None
    ]
    if not employees:
        return []
    ...
    all_skills = [
        s for s in session.exec(select(SkillTable)).all()
        if s is not None and getattr(s, "employee_id", None) is not None and getattr(s, "name", None)
    ]
    ...
    results.sort(key=lambda x: x["match_details"]["overall_compatibility"], reverse=True)
    return results[:5]
...
def evaluate_team(team: List[EmployeeTable], target_skills: List[Dict[str, Any]], budget_cap: float, session: Session, skills_by_employee: Dict[UUID, List[SkillTable]] = None) -> Tuple[float, Dict[str, Any]]:
    ...
    for emp in team:
        ...
        base_salary = 80000 + (len(emp.role) * 1500)  # Algorithmic pricing based on position
...
def optimize_team(payload: Dict[str, Any], session: Session = Depends(get_session), current_user: TokenData = Depends(get_current_user)):
    """
    ...
    if len(employees) < max_team_size:
        raise HTTPException(status_code=400, detail="Not enough employees in database to construct a team of requested size")
    ...
    current_energy, current_breakdown = evaluate_team(current_team, target_skills, budget_cap, session, skills_by_employee)
    ...
    while temp > min_temp:
        ...
        cand_energy, cand_breakdown = evaluate_team(candidate_team, target_skills, budget_cap, session, skills_by_employee)
...
def calculate_hazard_rate(session: Session = Depends(get_session), current_user: TokenData = Depends(get_current_user)):
    """
    ...
    for emp in employees:
        ...
        peak_1yr = math.exp(-0.5 * ((tenure_months - 12.0)/3.0)**2)  # Gaussian peak at 1 year
        peak_3yr = math.exp(-0.5 * ((tenure_months - 36.0)/6.0)**2)  # Gaussian peak at 3 years
        ...
        if overqualified:
            ...
            covariates.append({
                ...
                "val": len(skills),
                "impact_direction": "risky",
                "impact_percentage": round((math.exp(overqual_effect) - 1.0) * 100, 1)
            })
...
@router.get("/ona")
def compute_ona(session: Session = Depends(get_session), current_user: TokenData = Depends(get_current_user)):
    """
    ...
    # Pre-fetch all skills once and group by employee_id to avoid O(N^2) SQLite queries
    all_skills = [
        s for s in session.exec(select(SkillTable)).all()
        if s is not None and getattr(s, "employee_id", None) is not None and getattr(s, "name", None)
    ]
    ...
    nodes = []
    node_id_map = {emp.id: idx for idx, emp in enumerate(employees)}
    ...
    try:
        jira_logs = session.exec(select(IntegrationLogTable).where(
    ...
    for i in range(len(employees)):
        ...
        for j in range(i + 1, len(employees)):
            ...
            for log in jira_logs:
                ...
                if emp_a.email.lower() in details_lower and emp_b.email.lower() in details_lower:
                    collab_count += 1
            if collab_count > 0:
...
MARKOV_TRANSITIONS: Dict[str, Dict[str, float]] = {
    "Junior Software Engineer": {"Software Engineer": 0.75, "QA Engineer": 0.10, "Junior Software Engineer": 0.15},
    "Software Engineer": {"Senior Software Engineer": 0.60, "DevOps Engineer": 0.15, "Product Owner": 0.10, "Software Engineer": 0.15},
    "QA Engineer": {"QA Lead": 0.70, "Software Engineer": 0.20, "QA Engineer": 0.10},
    "DevOps Engineer": {"Senior DevOps Engineer": 0.70, "Cloud Architect": 0.20, "DevOps Engineer": 0.10},
    "Senior Software Engineer": {"Tech Lead": 0.45, "Engineering Manager": 0.35, "Solutions Architect": 0.15, "Senior Software Engineer": 0.05},
    "Tech Lead": {"Principal Engineer": 0.50, "Engineering Manager": 0.40, "Tech Lead": 0.10},
    "Engineering Manager": {"Director of Engineering": 0.80, "Engineering Manager": 0.20},
    "Director of Engineering": {"VP of Engineering": 0.90, "Director of Engineering": 0.10},
...
ROLE_SKILL_REQUIREMENTS: Dict[str, List[str]] = {
    ...
    "Senior Software Engineer": ["React", "TypeScript", "FastAPI", "System Design", "SQL"],
...
def get_markov_predictions(current_role: str, steps: int = 3) -> List[Dict[str, Any]]:
    ...
    current_key = next((k for k in MARKOV_TRANSITIONS if k.lower() == current_role.lower()), None)
    if not current_key:
    ...
    for step in range(1, steps + 1):
        ...
        for state, prob in state_probs.items():
            ...
            for next_state, trans_prob in transitions.items():
                next_probs[next_state] = next_probs.get(next_state, 0.0) + (prob * trans_prob)
                
        state_probs = next_probs
        
        # Sort and take top transitions
        sorted_transitions = sorted(state_probs.items(), key=lambda x: x[1], reverse=True)
        top_moves = []
        for role, p in sorted_transitions:
            if p > 0.05 and role != current_key:  # Filter noise & static self loops
                top_moves.append({
                    ...
                    "required_skills_missing": ROLE_SKILL_REQUIREMENTS.get(role, [])
                })
...
def predict_career_path(employee_id: UUID, session: Session = Depends(get_session), current_user: TokenData = Depends(get_current_user)):
    ...
    skills = [
        s for s in session.exec(select(SkillTable).where(SkillTable.employee_id == emp.id)).all()
        ...
        for move in step["predictions"]:
            ...
            for rs in req_skills:
                if rs.lower() not in has_skills:
                    ...
                    gaps.append({
                        "skill": rs,
                        "bridgeable_distance": round(min_dist, 2) if min_dist < 99 else None,
                        "difficulty": "Easy (Semantic Adjacent)" if min_dist <= 0.2 else 
                                      "Medium (Indirect Path)" if min_dist <= 0.6 else "Hard (New Skill Node)"
                    })
