...
def get_tools(api_key: str):
    ...
    def get_talent_pool(talent_type: str):
        ...
        with Session(engine) as session:
            if talent_type == "employee":
                ...
                for t in results:
                    skills = session.exec(
                        select(SkillTable).where(SkillTable.employee_id == t.id)
                    ).all()
                    out.append(
                        {
                            "id": str(t.id),
                ...
                for t in results:
                    skills = session.exec(
                        select(SkillTable).where(SkillTable.candidate_id == t.id)
                    ).all()
                    out.append(
                        {
                            "id": str(t.id),
    ...
    def analyze_department_sentiment(department: str):
        ...
        """
        with Session(engine) as session:
            statement = select(EmployeeTable).where(
    ...
    return [
        get_talent_pool,
        analyze_department_sentiment,
        search_candidates_semantically,
    ]
